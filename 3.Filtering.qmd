...
This removed `r nrow(sample_data(physeq)) - nrow(sample_data(physeq.filtered))` samples, and resulting data (phyloseq object) has `r nrow(sample_data(physeq.filtered))` samples. Samples removed in this step are shown below:
...
-   After removing taxa with zero counts (if any), we have `r ncol(otu_table(physeq.filtered))` taxa left.

## Removing artifacts

We need to remove ASVs that are likely artifacts, that could be sourced from host DNA or other sources. We should get rid of:

-   Unranked/unassigned <!-- -   Eukarya (kingdom): not for this dataset -->

### Unranked/unassigned

The RDP classifier and other classifiers will sometimes misclassify sequences that have very little sequence similarity to 16S. These tend to classify poorly by rank (NA or 'Unclassified' up to and sometimes including Domain/Kingdom).
...
Quite a bit is unassigned at the Species level `r ranks[7,]` but this is common to see as amplicon data usually cannot finely classify at the lower levels (compared to shotgun or long read data for example that can capture more variation). Genus and Family levels have less than `r max(ranks[5:6,])` unassigned. Phylum, Class and Order levels have less than `r max(ranks[1:3,])` unassigned. Other reasons for high percentages of unclassified at the lower taxonomic level are sample specific diversity and novelty, what is available in the database and whether exact matches can be found using DADA2 (requires exact match which is harder to find with amplicon data).
...
df.unclassified = df[df$Domain == "Unclassified",]
```

### Remove non-bacterial taxa

We have `r df.archaea$Count` Archaea. Let's remove them.
...
There are `r df.unclassified$Count` that are unclassified across almost all ranks; anything not classified at least to a phylum level are uninformative. We should go ahead and remove these.
...
After removing the unclassified phyla, we check for Eukaryotic reads but none are there.
...
Next we check for organelle 16S (i.e., chloroplast or mitochondria). They are classified under different rank criteria in Silva (Chloroplast is under the Order rank and Mitochondria under Family), so we need to check for them separately.
...
## Remove taxa or samples with 0 counts
...
Below are the sample variables for analysis. Note these include data from read tracking.
...
Here is a simple summary of the phylogenetic tree (this is midpoint-rooted in the workflow) with the top 50 most abundant ASVs at the family level. Same patterns here observed here similar to the raw composition plot as more similarities are observed between the wetting agent within the sampling methods.
...
sample_data(ex0)$group <- factor(sample_data(ex0)$group, levels = c("Drags_PBS","Drags_BPW","Booties_PBS","Booties_BPW","Grabs_N/A"))
