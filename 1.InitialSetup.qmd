...
In this data analysis, a microbial community analysis is performed to see if there are any additional differences between the sampling sites and methods. V3V4 region of 16S ribosomal gene was amplified and sequenced using the Fluidigm platform. There is a total of 144 samples. The factors are described below:

The two most important factors of interest are **Sample_Type** and **Wetting_Agent**.
...
tmp <- tmp[order(tmp$Wetting_Agent, tmp$Sample_Type, tmp$Commodity_Type), ]

# Create a group -----

tmp$group <- paste(tmp$Sample_Type,tmp$Wetting_Agent, sep = "_")
...
tmp$Label <- paste(tmp$group, tmp$Commodity_Type, sep = "_")
...
knitr::kable(head(tmp, n = 15), show_col_types = FALSE, row.names = F)
...
knitr::kable(track[c(1:20),c(10,9,3:8,11)])  %>%
...
There are no samples with < 1000 final reads. In fact, the lowest number of reads in any sample is `r min(df$seqtab.merged.nochim)`.
...
ReadFate$Label <- factor(track$Sequencing_ID, levels = track$Sequencing_ID)

rownames(ReadFate) <- track$Sequencing_ID # adjust rownames
...
p <- ggplot(df, aes(x = Label, y = pct, fill = forcats::fct_rev(Fate))) +
...
p <- ggplot(df, aes(x = Label, y = pct / 1e6, fill = forcats::fct_rev(Fate))) +
