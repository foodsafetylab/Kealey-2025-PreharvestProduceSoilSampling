...
Now I will run a mixed model to test significance of our variables while considering for variations by location. I use the lme() function in R to run a Linear Mixed-Effects Model for **Sample_Type x Wetting_Agent** group by considering Location as a random factor in the model. For non-normal metrics of alpha diversity, I also separately used glm() function with Location as a random factor in the model with poisson distribution.
...
- The **Sample_Type x Wetting_Agent** group is significant at p < 0.05.


```{r, warning=FALSE}
shan.lm <-lme(fixed=Shannon~group, data=merged, random= ~1|Location)
...
```{r, warning=FALSE}
shan.glm <- glmer(Shannon~group + (1|Location), family=poisson, data=merged)
...
#shan <-lme(fixed=Shannon~group, data=merged, random= ~1|Location)
summary(glht(shan.glm,linfct=mcp(group="Tukey"))) #pairwise contrasts for testing differences between factors with >2 levels
```
...
```{r, warning=FALSE}
obs.lm <-lme(fixed=Observed~group, data=merged, random= ~1|Location)
Anova(obs.lm, type = "III")
#summary(shan) #model output, gives correlations between factors and maximum likelihood results
```
...
```{r, warning=FALSE}
summary(glht(obs.lm,linfct=mcp(group="Tukey"))) #pairwise contrasts for testing differences between factors with >2 levels
```
...
```{r, warning=FALSE}
obs.glm <- glmer(Observed~group + (1|Location), family=poisson, data=merged)
Anova(obs.glm, type = "III")
```
...
#shan <-lme(fixed=Shannon~group, data=merged, random= ~1|Location)
summary(glht(obs.glm,linfct=mcp(group="Tukey"))) #pairwise contrasts for testing differences between factors with >2 levels
```
...
```{r, warning=FALSE}
chao.lm <-lme(fixed=Chao1~group, data=merged, random= ~1|Location)

Anova(chao.lm, type = "III")
#summary(shan) #model output, gives correlations between factors and maximum likelihood results
```

group is not significant at p < 0.05.
...
summary(glht(chao.lm,linfct=mcp(group="Tukey"))) #pairwise contrasts for testing differences between factors with >2
```

### Generalized Linear regression with poisson distribution

```{r, warning=FALSE}
obs.glm <- glmer(Chao1~group + (1|Location), family=poisson, data=merged)
Anova(obs.glm, type = "III")
```
...
summary(glht(obs.glm,linfct=mcp(group="Tukey"))) #pairwise contrasts for testing differences between factors with >2
```
...
- Sample_Type and Wetting_Agent group is significant at p < 0.05.

```{r, warning=FALSE}
simpson.lm <-lme(fixed=Simpson~group, data=merged, random= ~1|Location)

Anova(simpson.lm, type = "III")
#summary(shan) #model output, gives correlations between factors and maximum likelihood results
```
...
summary(glht(simpson.lm,linfct=mcp(group="Tukey"))) #pairwise contrasts for testing differences between factors with >2 levels
```
...
```{r, warning=FALSE}
simpson.glm <- glmer(Simpson~group + (1|Location), family=poisson, data=merged)
Anova(simpson.glm, type = "III")
```
...
summary(glht(simpson.glm,linfct=mcp(group="Tukey"))) #pairwise contrasts for testing differences between factors with >2 levels
```
...
```{r, warning=FALSE}
pd.lm <-lme(fixed=PD~group, data=merged, random= ~1|Location)

Anova(pd.lm, type = "III")
#summary(shan) #model output, gives correlations between factors and maximum likelihood results
```
...
summary(glht(pd.lm,linfct=mcp(group="Tukey"))) #pairwise contrasts for testing differences between factors with >2 levels
```
...
```{r, warning=FALSE}
pd.glm <- glmer(Simpson~group + (1|Location), family=poisson, data=merged)
Anova(pd.glm, type = "III")
```
...
summary(glht(pd.glm,linfct=mcp(group="Tukey"))) #pairwise contrasts for testing differences between factors with >2 levels
```
